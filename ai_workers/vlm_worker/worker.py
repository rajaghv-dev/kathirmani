"""vlm_worker loop (master plan Phase 6 / spec/11 "don't use the VLM for everything").

Chains *after* the rule_engine: that worker republishes higher-level §8.3
        ...
        while True:
            ...
            for job_id, ev in jobs:
                try:
                    ...
                    print(f"[vlm_worker] event failed: {e}")
                    if is_pg and job_id is not None:
                        queue.fail(job_id)
            if once or not jobs:
                if once:
...
if __name__ == "__main__":                                   # python -m worker / direct
    import argparse
    ap = argparse.ArgumentParser(description="vlm_worker")
    ...
    print(f"[vlm_worker] processed {len(res)} event(s), verified {verified}: {res}")
