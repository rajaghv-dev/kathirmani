"""rule_engine worker loop (master plan Phase 5).
...
def _write_event(conn, ev: dict) -> str | None:
    """Insert a rule_engine CommonEvent into `events`; returns event_id or None."""
    if conn is None:
        return None
        ...
        while True:
            ...
            for job_id, ev in jobs:
                try:
                    ...
                    print(f"[rule_engine] event failed: {e}")
                    if is_pg and job_id is not None:
                        queue.fail(job_id)
            if once or not jobs:
                if once:
...
if __name__ == "__main__":                           # python -m worker / direct
    import argparse
    ap = argparse.ArgumentParser(description="rule_engine worker")
    ...
    print(f"[rule_engine] processed {len(res)} event(s), fired {fired} action(s): {res}")
