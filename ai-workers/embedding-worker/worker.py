"""embedding-worker CLI (master plan Phase 8).
...
if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(description="embedding-worker")
    ...
    if args.cmd == "index":
        ...
        print(f"[embedding-worker] indexed: {counts}")
    elif args.cmd == "query":
        results = query_run(args.text, k=args.k)
        print(f"[embedding-worker] {len(results)} hit(s) for {args.text!r}:")
