...
--
-- We introduce a NOLOGIN *group* role `marlin_app` that the application's login
-- role(s) join (GRANT marlin_app TO <login_role>). The group holds exactly
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'marlin_app') THEN
    CREATE ROLE marlin_app NOLOGIN;
  END IF;
END
$$;
...
GRANT INSERT, SELECT ON audit_log TO marlin_app;
...
--   GRANT marlin_app TO <app_login_role>;
