...
## The detection layer — `kathirmani.device`

`src/kathirmani/device.py` is the single source of truth. It splits into:
...
```python
from kathirmani.device import detect_device
cfg = detect_device()
print(cfg.describe())
...
Set the **`KATHIRMANI_DEVICE`** environment variable (or pass `prefer=`) to one of
...
```bash
KATHIRMANI_DEVICE=cpu bash run.sh --duration 10   # force CPU for a quick smoke test
```
...
`kathirmani/pipeline.py::load_model` (Marlin-2B) and
`kathirmani/qwen_vl.py::load_qwen_model` (Qwen2.5-VL) both follow this pattern.
...
`kathirmani/metrics.py` polls hardware telemetry. The NVIDIA-specific sources are
now guarded:
...
`kathirmani/ffmpeg_preload.py::preload_av_ffmpeg` dlopens PyAV's bundled FFmpeg
...
| `run_inference.py` hardcoded `python3.12` av.libs preload | `kathirmani.ffmpeg_preload`, version-globbed, Linux-guarded |
