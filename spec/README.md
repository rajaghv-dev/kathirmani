# Kathirmani — Spec Documentation

This spec/ directory is the reorganized, structured documentation for the Kathirmani video-intelligence pipeline. It is derived from learning.md (kept at repo root as the original working notes).
...
| [09-repo-structure.md](09-repo-structure.md) | The `src/kathirmani` package layout, the root entry-point shims, path anchoring, and how every command stays unchanged. |
...
| [18-multiview-tiered-inference.md](18-multiview-tiered-inference.md) | **Design (measured).** Same room · 5 angles · one GPU: the tier hierarchy (motion gate → view election → small detector → rules/SQL fusion → Cosmos triage → budgeted Nemotron verdicts), same-room optimizations ranked by measured ROI, hot/cold operating modes, implementation status. |
| [14-plug-and-play-quality-pos-observability.md](14-plug-and-play-quality-pos-observability.md) | **Forward-looking implementation prompt.** Generalizes WHEN/WHERE/WHAT into plug-and-play model hooks, adds a QUALITY ingestion hook, POS-aware correlation, OpenTelemetry, Netdata resource profiling, model routing, and small-model parity reporting. |
